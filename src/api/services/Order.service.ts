...
export class OrderService {
    ...
    public sendOrder(order: Order, orderFile: File[]): Promise<Order | undefined> {
     ...
     * @param data File[]
     */
    public prepareAttachments(data): attachmentsInterface {
        return { attachments: data.map((element, index) => {
            return {
    ...
    public sendMailOrder(order: Order, orderImage: any): void {
        ...
        this.mailService.send({ to, from, subject, attachments }, 'order', { message });
    }
}
